        ...
        if (selected.val() == item.name) {
          ...
          if (item.imgColorDescription != "") {
            $("#descriptionValue").append(
              "<br/><img id='imgDescription' src='" +
                item.imgColorDescription +
                "' />"
            );
            thereIsImage = true;
          ...
          if (item.metrics.length > 0) {
            ...
            for (i = 0; i < item.metrics.length; i++) {
              output +=
                "<tr><td><span class='text-info' rel='popover' data-trigger='click' data-placement='bottom' data-html='true' data-original-title='Metric description' data-content='" +
            ...
            output += "</table>";
            output +=
              "<script type='text/javascript'> $('[rel=\"popover\"]').popover();</script>";
...
function paintViewer(gexfPath) {
  viewer = sigma
    .init(document.getElementById("sigma-example"))
    .drawingProperties({
...
function fishEye(gexfPath) {
  ...
  (function () {
    ...
    var FishEye = function (sig) {
      ...
      this.p = {
        // The object containing the properties accessible with
        radius: 200, // the Cascade.config() method.
        power: 2,
      };

      function applyFishEye(mouseX, mouseY) {
        // This method will apply a formula relatively to
        // the mouse position.
        var newDist,
          newSize,
          xDist,
        ...
        sig.graph.nodes.forEach(function (node) {
          ...
          if (dist < radius) {
            ...
            if (!node.isFixed) {
              node.displayX =
                mouseX + xDist * (((newDist / dist) * 3) / 4 + 1 / 4);
              node.displayY =
                mouseY + yDist * (((newDist / dist) * 3) / 4 + 1 / 4);
            }

            node.displaySize = Math.min(
              (node.displaySize * newSize) / dist,
              10 * node.displaySize
            );
          }
        });
    ...
    sigma.publicPrototype.activateFishEye = function () {
      ...
      if (!this.fisheye.activated()) {
        ...
        document
          .getElementById("sigma_mouse_" + this.getID())
...
function search(nodeName) {
  ...
  viewer.iterNodes(function (n) {
    ...
    if (
      n.label.toLowerCase().substring(0, nodeName.length) ===
